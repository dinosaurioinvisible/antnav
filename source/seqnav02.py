...
class Seq2SeqPerfectMemory:
    ...
    # updates mem window with incoming query image
    def mk_query_window(self, query_img, centred=True, db=0, df=10):
        # expanded window
        blw = max(0, self.blimit - db)
        flw = min(self.flimit + df, len(self.route_images)-1)
        wrsims = rmf(query_img=query_img, ref_imgs=self.route_images[blw:flw])
        # query id
        indices = self.argminmax(wrsims, axis=1)
        wind_sims = [wrsims[i,idi] for i,idi in enumerate(indices)] 
        idq = self.blimit + int(round(self.argminmax(wind_sims)))
        # make window
        idb, idf = min(self.mem_pointer,idq), max(self.mem_pointer,idq)
        idf = int(round((idb+idf)/2)) if centred == True else idf
        self.flimit = min(idf+11, len(self.route_images)-1) if idb > 0 else idf + self.window
        self.blimit = max(0, self.flimit - self.window)
    
    # defines a reduced mem window from temp contrast
    def mk_sub_window(self, query_seq, wsize=3):
        # get temp contrasts difs (intensity wise)
        # non abs so is independent from rotation
        subw_ids, qr_sums = [], []
        for wi in range(self.blimit,self.flimit-wsize):
            qr_dif = np.array(self.route_images[wi:wi+wsize]) - np.array(query_seq)
            subw_ids.append((wi,wi+wsize))
            qr_sums.append(np.sum(qr_dif))
        # more similar changing coef
        qr_min = np.min(np.abs(qr_sums))
        qr_id = int(np.where(np.abs(qr_sums)==qr_min)[0])
        subw_blim, subw_flim = subw_ids[qr_id]
        # update pointer and (sub-) window
        self.mem_pointer = subw_flim -1     # just centred mp, for speed
        self.blimit = max(0,subw_blim-wsize)
        self.flimit += wsize
    ...
    def get_heading(self, query_img):
        ...
        self.query_window = True
        if len(self.queue) == 3 and self.query_window == True:
            self.mk_query_window(query_img, db=0, df=10, centred=True)
        
        self.sub_window = True
        if len(self.queue) == 3 and self.sub_window == True:
            self.mk_sub_window(self.queue, wsize=3)
        ...
        if not self.sub_window:
            ...
            # mem pointer gets updated independently from heading
            self.flimit = min(self.mem_pointer+11,len(self.route_images)-1) if self.blimit > 1 else self.mem_pointer+self.window
            self.blimit = max(1,self.flimit - self.window) 
